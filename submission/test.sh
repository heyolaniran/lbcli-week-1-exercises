...
COLLECTED=$($NEW_BALANCE - $BALANCE | bc)
...
# WRITE YOUR SOLUTION BELOW:
VERIFY_RESULT=$(bitcoin-cli -regtest -rpcwallet=btrustwallet verifymessage $LEGACY_ADDR "$SECRET_MESSAGE")
...
# WRITE YOUR SOLUTION BELOW:
INTERNAL_KEY=$($ADDR_INFO | jq -r .desc | cut -d "]" -f 2 | cut -d ")" -f 1)
...
# WRITE YOUR SOLUTION BELOW:
DERIVED_ADDR_RAW=$(bitcoin-cli -regtest deriveaddress $TAPROOT_DESCRIPTOR)
